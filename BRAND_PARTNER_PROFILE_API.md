...
## Member Since Field Details

### Purpose
The "memberSince" field automatically captures the date when the brand partner account was created. This helps track partnership duration and can be displayed on the profile.

### Characteristics
- **Auto-Generated**: Automatically set when brand partner is created
- **Default Value**: Current date/time (`Date.now()`)
- **Editable**: No - cannot be modified after creation
- **Format**: ISO 8601 date-time string
- **Use Cases**: 
  - Display "Member since [date]" on profile
  - Calculate partnership duration
  - Show tenure badges (e.g., "5+ years partner")
  - Sort partners by seniority
...
const yearsAsMember = Math.floor(
  (Date.now() - new Date(profile.memberSince)) / (1000 * 60 * 60 * 24 * 365)
)
// Output: 2 (years)
```
...
1. **Profile Page**
   ...
   - Display "Member since [date]" badge
...
// Calculate years as member
const calculateYearsAsMember = (dateString) => {
  const memberDate = new Date(dateString)
  const years = Math.floor(
    (Date.now() - memberDate) / (1000 * 60 * 60 * 24 * 365)
  )
  return years
}
...
// Example JSX for displaying member since
<div className="profile-header">
  <h2>{profile.partnerName}</h2>
  <span className="partner-code">{profile.partnerCode}</span>
  <span className="member-badge">
    Member since {formatMemberSince(profile.memberSince)}
  </span>
  {calculateYearsAsMember(profile.memberSince) >= 5 && (
    <span className="veteran-badge">5+ Years Partner</span>
  )}
</div>
```
...
The following fields are added to the BrandPartner model:
...
- **about**: Editable text field for brand partner's business description
- **memberSince**: Auto-generated timestamp when the brand partner account is created
