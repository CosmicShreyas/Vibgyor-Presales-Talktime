...
export class NativeCallService implements OnDestroy {
  ...
  private restoreSession(): void {
    ...
    try {
      ...
      if (!session?.clientId || !session?.launchedAt) return;
      this.activeSession.set(session);
  ...
  private syncElapsed(): void {
    ...
    this.elapsedSeconds.set(Math.max(0, Math.floor((endTime - session.startedAt) / 1000)));
  }

  private handleDialerHidden(): void {
    const session = this.activeSession();
    if (!session || session.startedAt || session.endedAt) return;

    // Start timing only if the browser hands off quickly after launching the tel link.
    if (Date.now() - session.launchedAt > 15000) return;
  ...
  private handleDialerReturn(): void {
    const session = this.activeSession();
    if (!session || !session.startedAt || session.endedAt || this.logging()) return;
    ...
    void this.autoLogCompletedCall(updated, duration);
  }

  private async autoLogCompletedCall(session: ActiveCallSession, duration: number): Promise<void> {
    this.logging.set(true);
    this.lastError.set(null);
    ...
    try {
      ...
      const message =
        (err as { error?: { message?: string } })?.error?.message || 'Unable to save this call right now.';
      this.lastError.set(message);
      console.error('Auto-log call failed:', err);
    } finally {
      this.logging.set(false);
      this.clearSession();
    }
